...
| ~~C-7~~ (ask #16, 2026-09-22) | ~~The source Excel's "Téma besorolás" sheet checked for survey questions missing from the parsed data~~ — **resolved 2026-09-25, D40**: the answers were in the data for every area all along; four topic rows were missing and are now in `IDBCSYNC` | `SURVEY-TOPIC…` rows in `IDBCSYNC` | nothing pending |
...
| C-10 (2026-09-25) | The `Non-IT / Qualified Person` Expert Pool row has no headcount | `IDBCSYNC` `EXPERT-27-DARAB` — the tile appears on the next sync once the number is typed in | the tile is left out (not invented, not crashed on); asked in the sheet's "HIÁNYZÓ ADATOK - kérés" tab (since D45 the row is hidden with the other Expert Pool tile rows — show the rows to type the number) |
