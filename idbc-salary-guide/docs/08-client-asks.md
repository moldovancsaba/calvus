...
| 17 | "IT Contracting" as a Bérek salary segment — the bértábla sheet has no rows for it (the separate market-trends survey data does have it) | IT Contracting salary figures from the client, or confirmation the segment is already folded into "IT" today. Code is ready (`build-salary-data.py`'s `AREA_NAME`); it will just show zero rows until data lands. |
