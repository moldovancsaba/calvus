...
| ADR-2 | Build-time data import, not a runtime Sheets API | runtime API (the spec) | §3: data is periodic and needs transformation; the converters exist and are deterministic. A scheduled import that runs the same converters is the upgrade path if the client wants "edit the Sheet, see it live". |
