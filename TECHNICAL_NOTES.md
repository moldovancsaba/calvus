# 👇 This file has been modularized as of version 2.0 — Sections below are now in dedicated Markdown docs under /documents:

- [Technical Architecture](technical-architecture.md)
- [Performance Optimizations & Edge Cases](performance-optimizations.md)
- [Security & Scalability](security-scalability.md)

## Quick Reference Index:

- [User Guide (README)](README.md)
- [Technical Architecture](technical-architecture.md)
- [Performance Optimizations](performance-optimizations.md)
- [Security & Scalability](security-scalability.md)
- [Changelog](CHANGELOG.md)

> **TIP:** All files above are available under `/documents` for fast navigation and reading.

---

_Last updated: 2025-06-14_

<!--
NOTE: The base triangle mesh is 18 triangles, reflecting the geometry.ts implementation. Historical "20 triangle" references have been corrected across all documentation.
-->
