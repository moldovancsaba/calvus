...
is written on D1–D29. These are the items the implementation needs, grouped by when they
block.*
...
| P-3 (ask #6) | Legal review of the consent templates and the legitimate-interest assessment per market (D8, D13) — including the logic audit's A1, the legal basis for B2C e-mail in Hungary (D29: to counsel after acceptance; `consent` until then) — and the policy record (SSOT §6b) confirmed: unknown age = no profiled offer, the vulnerability pause, the postal address on letters, the seller's policy naming the processor | product owner, counsel | E6; the gate reads the record |
