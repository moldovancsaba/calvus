...
STEPS = [
 ...
 ("4", "Rendszeres gondozás", "Kéthetente vagy hetente jövünk: víz, tápanyag, metszés, kártevő-ellenőrzés, tisztítás. Minden látogatás után egy kártya az asztalon. A gyakornok szabadnapos."),
]
FAQ = [
 ...
 ("Mi van, ha egy növény mégis elpusztul?", "Cseréljük, díjmentesen. Ez az örökzöld garancia: ami a mi gondozásunk alatt leépül, azt mi pótoljuk. Nincs vita, nincs jegyzőkönyv."),
...
TEAM = [("[Név]", "alapító, ügyvezető", "Tíz éve zöldfalakat és irodákat építünk Grofie néven. Ez a csapat annak a része, amelyik az irodákra szakosodott."), ("[Név]", "növénygondozó, a te irodád felelőse", "Kéthetente ott van. Név szerint ismeri a fikuszokat."), ("[Név]", "tervező", "A három fotóból látványterv. Két munkanap.")]
...
<div><b>10+</b><span>év tapasztalat</span></div><div><b>100+</b><span>megvalósított projekt</span></div><div><b>30+</b><span>gondozott iroda</span></div><div><b>100%</b><span>örökzöld garancia</span></div>
<small>A Grofie (Gourmet Garden Kft.) számai — ugyanaz a csapat, az irodákra szakosodva.</small>
</div></section>'''

def offers():
    return f'''<section class="sec" id="szolgaltatasok"><div class="wrap"><div class="head"><div><p class="kicker">Három út a zöld irodához</p><h2>Vétel, bérlés vagy csak gondozás</h2></div></div>
<div class="offers">
<article class="offer">{img("nyitott-iroda", "Nyitott iroda növénykaspókkal az asztalsorok között", sizes="(min-width:768px) 33vw, 100vw")}<p class="fun">Mert a csupasz iroda már 2018-ban is ciki volt</p><p class="pro">Felmérjük a fényt, a klímát, a használatot; megtervezzük, telepítjük, gondozzuk. Egy kézben.</p><a href="irodai-novenyek.html">Növénydekoráció →</a></article>
...
<article class="offer">{img("lobby-narancs", "Recepció narancssárga fotelekkel és növényekkel", sizes="(min-width:768px) 33vw, 100vw")}<p class="fun">Növénybérlés, amíg az iroda még alakul</p><p class="pro">Havidíj, gondozással. Költözéskor jönnek veletek; rendezvényre egy napra is.</p><a href="novenyberles.html">Növénybérlés →</a></article>
</div></div></section>'''
...
<div><p class="kicker">Örökzöld garancia</p><h2 style="font-size:28px;margin-bottom:8px">Ami a mi gondozásunk alatt leépül, azt mi cseréljük. Díjmentesen.</h2><p class="lede" style="font-size:15px">Nincs vita, nincs jegyzőkönyv. Ez a különbség a „majd valaki locsolja” és a „mi felelünk érte” között.</p></div>
...
def packages(full=False):
    ...
    return f'''<section class="sec" id="arak"><div class="wrap"><div class="head"><div><p class="kicker">Mennyibe kerül?</p><h2>Csomagok és induló árak — mert a „kérj ajánlatot” nem ár</h2></div>{"" if full else '<a class="btn alt" href="arak.html">Minden részlet</a>'}</div>
...
def steps():
    ...
    return f'''<section class="sec" id="folyamat"><div class="wrap"><div class="head"><div><p class="kicker">Hogyan működik</p><h2>Négy lépés, amiből egy a tiéd</h2></div></div>
...
def references(full=False):
    ...
    return f'''<section class="sec" id="referenciak"><div class="wrap"><div class="head"><div><p class="kicker">Akik már ránk bízták a zöldet</p><h2>Irodák, ahol a növények már dolgoznak</h2></div>{"" if full else '<a class="btn alt" href="referenciak.html">Minden referencia</a>'}</div>
...
def quote_form():
    return f'''<section class="sec" id="ajanlat"><div class="wrap"><div class="head"><div><p class="kicker">Zöld érdekel?</p><h2>Dobáld be a képeidet, és visszajelzünk, hogyan lehet ebből egy jól működő növénydekoráció</h2></div></div>
...
def instagram():
    return f'''<section class="sec" id="instagram"><div class="wrap"><div class="head"><div><p class="kicker">@bizdrankazoldet</p><h2>Instagramon is ott vagyunk</h2></div><a class="btn alt" href="https://www.instagram.com/bizdrankazoldet/">Instagram</a></div>
...
PAGES = {
 ...
 "irodai-novenyek.html": lambda: service("irodai-novenyek.html", "Irodai növénydekoráció tervezéstől gondozásig — Bízd ránk a zöldet!", "Irodai növénydekoráció: felmérés, tervezés, telepítés és gondozás egy kézben. A csupasz iroda 2018-ban is ciki volt.", "iroda-piros-2", "Iroda piros keretekkel és növénykaspókkal", "Irodai növénydekoráció", "A csupasz iroda már 2018-ban is ciki volt", "A fehér fal, a szürke padló és a sorban álló asztalok nem minimalizmus, hanem üresség. Néhány jól elhelyezett növény már elég, hogy az iroda ne két meeting közti átmeneti hely legyen.",
   [("Spoiler: nem minden növény akar az irodádban élni", "Nem a katalógusból indulunk ki, hanem a fényből, a klímából és abból, hogy ki mennyit foglalkozik vele. Olyat választunk, ami bírja a légkondit és a kevesebb fényt."), ("A pénzügyes otthoni kaspója vs. „ez így összeállt”", "Otthon belefér az impulzus; az irodában rendszerben gondolkodunk: méret, forma, elhelyezés, kaspó — a munkáltatói márka arculatával összehangolva."), ("Kicsi iroda, nagy iroda, startup iroda, tökmindegy, a zöld mindig működik", "Nem a négyzetméter dönt, hanem az arány. Egy jól elhelyezett állónövény megnyit egy sarkot; a zöld irányítja a tekintetet, nem elvesz a térből.")], [0, 1, 3, 5]),
