...
| `business-direct/` | A standalone product: the one-person sales and marketing team — automation for the founder, owner or operator who is the whole department. Seven departments as automations, the operator's knowledge as plain files, a lawful sales pipeline, conversations answered within the hour, a human gate on everything, one loop that ranks the next dollar, a Monday recap; one responsible-data policy record per customer. First paying customer: ClassScout (Your Field NYC); `index.html` is the clickable prototype on their real data (253 providers from `data/fetch-yourfield.py`; nothing sends). `docs/` holds the product presentation (`presentation.html`), the executive summary and the business case, six research rounds, the audit of the first customer's platform, decisions, the design system and layout frames, build log, gate, business logic, the technical package (SSOT, architecture, technical design, plan in sprints, token map, system blueprint), analytics, the audit and SWOT, the responsible-data policy framework, the first-customer prerequisites and the documentation audit | Every stage done 2026-09-19; the real-system blueprint and the documentation audit 2026-09-20 (D37–D39); the owner sets the price and the rate; nothing open for the presentation |
