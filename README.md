...
| `nepszabadsag/` | Népszabadság — the relaunch of the daily as an online paper with a weekly print edition, launching 2026-10-08: the pre-registration landing page (the client's memo, live countdown) and the site (Címlap, the Belföld rovatfront, one Cikkoldal with its paywall mechanic, Népszava almárka), generated from one content source (`build.py`/`content.py`) with real headlines read from the client's Figma file and fourteen real licensed photographs. `docs/` holds the brief, research, audit, sources, decisions, the design system as built, the build log, the gate, and the Hungarian presentation (`bemutato.html`) | First version built 2026-09-21, measured clean at 390 and 1440; next: rovat fronts for the remaining categories and the technical package |
