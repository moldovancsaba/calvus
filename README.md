...
| `bizdrankazoldet/` | Bízd ránk a zöldet — the playful office-plant brand of Gourmet Garden Kft. / Grofie (Budapest): office plant design, rental and care, maintenance included. `docs/` holds the brief, the research (trust, reliability, identity, the customer journey, the EU green-claims rules from 27 Sept 2026, five competitors measured; proposals P1–P12), the audit of the site, the Instagram account and the parent brand, and the sources | Prototype built 2026-09-20 from the client's own material (kit, photographs, illustrations, Reels): seven Hungarian pages, phone and desktop designed separately, presentation in Hungarian; next: the client's material for the samples |
| `nepszabadsag/` | Népszabadság — the relaunch of the daily as an online paper with a weekly print edition, launching 2026-10-08: the pre-registration landing page (the client's memo, live countdown) and the site (Címlap, the Belföld rovatfront, one Cikkoldal with its paywall mechanic), generated from one content source (`build.py`/`content.py`) with real headlines read from the client's Figma file and ten real licensed photographs. The Figma also designs a Népszava sub-brand front, deliberately not built, at the owner's instruction. `docs/` holds the brief, research, audit, sources, decisions, the design system as built, the build log, the gate, and the Hungarian presentation (`bemutato.html`) | First version built 2026-09-21, measured clean at 390 and 1440; next: rovat fronts for the remaining categories and the technical package |
| `gameformative/` | gameformative.com — a sport analytical and educational infotainment site (news, tactics, techniques, sport science and the business of sport, organised in eight desks — Discover, Define, Design, Develop, Data, Drive, Defend, Deal), launching with articles only. Every article: 800–3,200 characters, headed segments, the sources used and the sources investigated but not used — enforced by `build.py` and by `check.py` on the rendered pages. Pages from one generator (`build.py` + `content.py`); the round-1 football statistics (openfootball, CC0, through `data/convert.py` with asserts) are kept live as a later phase. `docs/` holds the research on the sport media industry, the named global sites and the top sites of Germany, India and China, 2027 colour forecasts and UX practice, **a register of every source researched with what was learned**, the audit, design system, decisions, gate, the technical package and the English `presentation.html` | Round 2 built 2026-09-25: the owner's first article (sources checked), five data articles, eight desks, measured clean at 375 and 1440; next: the owner's review of the direction |
