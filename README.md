...
| `business-direct/` | Automated marketing machine for B2B2C listing platforms: B2C social publishing to acquire families for the listing page and a B2B sales pipeline to bring providers to manage their own page and more; platform, provider and family views; optional AI; intelligence dashboard; integrations. First client ClassScout (operator of Your Field NYC) — 252 real providers pulled from its public API by `data/fetch-yourfield.py` (the Hungarian sport platform kept as reference data). Sibling of DiscountDirect. `index.html` is the clickable prototype (three views, in-memory state, nothing sends); `docs/` holds the presentation, brief, four research rounds, audit, decisions, the approved design system and layout frames, build log, gate, business logic, the technical package (SSOT, architecture with PROPOSED ADRs, technical design, plan, token map), analytics, the audit and SWOT, the responsible-data policy framework and the implementation prerequisites | Every stage done 2026-09-19: gates 1–2 approved, nine build rounds, presentation, business logic, technical package (stack PROPOSED), audit and SWOT, responsible-data policy record and gate, Simple and Advanced interfaces; nothing open for the presentation |
