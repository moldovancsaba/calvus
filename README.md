...
| `business-direct/` | Your Field NYC (ClassScout) lists 253 children's-activity providers and none manages its page — enquiries unanswered, sessions and trials published nowhere, families told nothing. business.direct is the marketing and sales machine the platform runs on its own data: real content brings families to the listing page, a lawful, capped invitation brings providers to manage their page, the provider's own one-person team works behind it, one loop decides the next dollar, a person approves everything, a child is an age and never a name. `index.html` is the clickable prototype on the real catalogue (253 providers from `data/fetch-yourfield.py`; nothing sends); `docs/` holds the client presentation (`presentation.html`), the executive summary and the business case, six research rounds, the audit of the platform, decisions, the design system and layout frames, build log, gate, business logic, the technical package (SSOT, architecture, technical design, plan in sprints, token map, system blueprint), analytics, the audit and SWOT, the responsible-data policy framework, the implementation prerequisites and the documentation audit | Every stage done 2026-09-19; the real-system blueprint and the documentation audit 2026-09-20 (D37, D38); nothing open for the presentation |
