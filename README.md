...
| `business-direct/` | Automated marketing machine for B2B2C listing platforms: B2C social publishing to acquire families for the listing page and a B2B sales pipeline to bring providers to manage their own page and more; platform, provider and family views; optional AI; intelligence dashboard; integrations. First client ClassScout (operator of Your Field NYC) — 253 real providers pulled from its public API by `data/fetch-yourfield.py` (the Hungarian sport platform kept as reference data). Sibling of DiscountDirect. `index.html` is the clickable prototype (three views, in-memory state, nothing sends); `docs/` holds the presentation, brief, six research rounds, audit, decisions, the approved design system and layout frames, build log, gate, business logic, the technical package (SSOT, architecture with the ADRs as the build baseline, technical design, plan in sprints, token map), analytics, the audit and SWOT, the responsible-data policy framework, the implementation prerequisites, and the real-system blueprint (services verified, modules with pseudo code) | Every stage done 2026-09-19; the real-system blueprint 2026-09-20 (D37): gates 1–2 approved, ten build rounds, presentation, business logic, technical package, audit and SWOT, responsible-data policy record and gate, Simple and Advanced interfaces, every service verified, modules and sprints; nothing open for the presentation |
