...
| `gameformative/` | gameformative.com — a football analytics and news site (results and tables for Europe's big five, season and World Cup reviews, analysis built on the numbers, a method page), on real public-domain data (openfootball, CC0) through a converter with asserts (`data/convert.py`) and one generator (`build.py` + `content.py`). `docs/` holds the research on the sport media industry, the named global sites and the top sites of Germany, India and China, 2027 colour forecasts and UX practice, **a register of every source researched with what was learned**, the audit, design system, decisions, gate, the technical package and the English `presentation.html` | First version built 2026-09-25: 19 pages, measured clean at 375 and 1440, light and dark; next: the owner's review of the direction |
