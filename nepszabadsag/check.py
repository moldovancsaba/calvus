...
6. every image has alt text; 7. every site page carries the prototype banner."""
import re, sys, pathlib
HERE = pathlib.Path(__file__).resolve().parent; ROOT = HERE.parent
SITE = sorted(p for p in HERE.rglob("*.html") if "docs" not in p.parts)
...
for p in DOCS:
    if p.stem == "bemutato": continue  # the presentation carries no docs nav, per the standard
