...
**2026-09-26 — round 5: the client's review.** The client: "great for the first run" — and asked for
developer-ready documentation (architecture, low-level design with pseudo code, AI-search, social and
messaging sharing, full SEO, backlinking) and a scientific source catalogue reachable from the
articles. Delivered first in the prototype: the source catalogue (D32), `noindex` + canonical on the
prototype (D33), share-ready pages, images, icons, sitemap, feed, robots.txt, llms.txt (D34); gate
checks 12b and 13. The developer documentation follows once four research rounds (AI search,
sharing, SEO and links, services) are verified.
